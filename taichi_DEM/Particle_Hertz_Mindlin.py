...
class ParticleHertzMindlin:
    def __init__(self, args):
        ...
        ##经过后面的代码阅读，我发现所谓分数系数其实是摩擦系数，这里是输入错误。
    ...
    def shake_wall(self):
        ...
        #     print(self.tik_sys[0].tik)
        if self.tik_sys[0].tik > self.quarter_period: ## 这里看到，每当时间过去了四分之一周期，这里就变为零，应该是实现了一个计时的功能
            self.tik_sys[0].tik = 0
            ti.atomic_add(self.tik_sys[0].shake_sate_machine, 1) ## 同时，这里又将shake_sate_machine加一
            if self.tik_sys[0].shake_sate_machine >= 4: ## 到这里shake_sate_machine的意思就很明显了，这应该是打错字了，应该是shake_state_machine
                self.tik_sys[0].shake_sate_machine = 0 ## 振动循环有四个状态，当循环到第四个状态时，状态就归零
        ## 至此我已经可以理解这一段代码的作用了

        for j in self.wf:
            if self.tik_sys[0].shake_sate_machine == 0 or self.tik_sys[0].shake_sate_machine == 3:
                self.wf[j].speed += self.a_shake ## 状态0、3时，加速度是正向的，总体具有加速和减速过程，应该是为了尽量接近真实物理情况
                sign = tm.sign(tm.dot(self.direction, self.wf[j].normal)) ## 计算速度和法线的点积，以此判断速度是否和壁面法线相反，从而给出0、3状态中的不同速度算法
                self.wf[j].distance -= sign * tm.dot(self.direction * self.wf[j].speed,
                                                     self.wf[j].normal)
            else:
                self.wf[j].speed -= self.a_shake ## 在另外两个状态下，加速度反向，同上理
    ...
    def resolve(self, i, j):
        ...
        ## 接下来这一段涉及到粒子接触时的Hertz-Mindlin算法
            ...
            beta = (1.0 / tm.sqrt(1.0 + (tm.pi / ti.log(self.restitution_coef)) ** 2)) ## 计算\beta系数
            
            e_star = 2.0 * self.elasticModulus / (1.0 - self.poissonRatio ** 2) ## 计算等效弹性模量
            g_star = self.elasticModulus / ((2.0 - self.poissonRatio) * (1.0 + self.poissonRatio)) ## 计算等效剪切模量
            r_star = 1.0 / (1.0 / self.particles[i].radius + 1.0 / self.particles[j].radius) ## 计算等效半径
            m_star = 1.0 / (1.0 / self.particles[i].mass + 1.0 / self.particles[j].mass) ## 计算等效质量

            kn = 4.0 / 3.0 * e_star * tm.sqrt(r_star * delta) ## 计算法向的等效弹性系数
            cn = 2.0 * beta * tm.sqrt(5.0 / 6.0 * kn * m_star) ## 计算法向阻尼力系数
            Fn = kn * delta * normal - cn * rel_vn ## 计算法向力，算法为法向弹性系数乘以接触点坐标与粒子i的相对位置，再减去法向阻尼力系数乘以相对法向速度

            ks = 8 * g_star * tm.sqrt(r_star * delta) ## 计算切向的等效弹性系数
            cs = 2.0 * beta * tm.sqrt(5.0 / 6.0 * ks * m_star) ## 计算切向阻尼力系数
            shear_increment = rel_vt * self.dt ## 计算切向增量
            self.cf[offset].shear_displacement += shear_increment ## 将切向增量叠加到总切向增量中
            Fs_try = ks * tm.dot(self.cf[offset].shear_displacement, tangent) * tangent ## 计算切向接触力
            Fs = Fs_try - cs * rel_vt ## 计算切向接触力，算法为切向弹性系数乘以切向增量，再减去切向阻尼力系数乘以相对切向速度

            Fn_abs = tm.length(Fn) ## 两个方向上力的大小
            Fs_abs = tm.length(Fs)

            ## 这一段我已经在Obsidian中打出公式，一些部分和Hertz-Mindlin的公式重合，但是有些变形需要再思考思考
    ...
    def evaluate_wall(self, i, j):
        ...
        if gap < 0:
            ...
            ## 这里将接触点视为粒子与壁面重合部分的中心点，因此计算出接触点与粒子i中心的相对位置 
    ...
    ## 这里我有一个疑问，判断是否接触似乎有两重标准，一个是粒子与壁面接触，另一个是粒子与壁面接触的间隙大小是否大于0，为什么需要这样？    @ti.kernel
    def resolve_wall(self):
        n_p, n_w = self.particle_num, self.n_walls ## 给出遍历范围
        for i, j in ti.ndrange(n_p, n_w): ## 二维数组遍历
            if self.wcf[i, j].isActive: ## 与边界接触有效
                dis = ti.abs(tm.dot(self.particles[i].pos, self.wf[j].normal) - self.wf[j].distance)
                if dis >= self.particles[i].radius: ## 此种情况下粒子与壁面不接触，因此各种参数直接归零
                    ...
                    self.evaluate_wall(i, j) ## 实际上接触，则调用上面的函数计算
            else:
                dis = ti.abs(tm.dot(self.particles[i].pos, self.wf[j].normal) - self.wf[j].distance)
                if dis < self.particles[i].radius: ## 粒子接触，那么计算接触力
    ...
    @ti.kernel
    def get_energy(self) -> ti.f32: ## 调用函数计算粒子能量，并返回能量值
        energy = 0.0
        for i in self.particles:
    ...
    @ti.kernel
    def get_average_ang_vel(self) -> ti.f32: ## 计算平均角速度，并返回
        average_ang_vel = 0.0
        for i in self.particles:
    ...
    @ti.kernel
    def get_abs_ang_vel(self) -> ti.f32: ## 计算角速度的大小（绝对值）
        abs_ang_vel = 0.0
        for i in self.particles:
    ...
    @ti.kernel
    def apply_fraction(self): ## 计算摩擦力，并作用到粒子上
        for i in self.particles: ## 遍历每一个粒子
            force_abs = tm.length(self.particles[i].force) ## 计算力的大小
            # print(f"robot {i} force: {force_abs}") ## 这行代码被注释掉了，如果没有注释掉，其作用是打印出每个粒子的力的大小
            if (force_abs <= self.particles[i].mass * self.gravity * self.fraction_coef)\
                    and (tm.length(self.particles[i].vel) < 0.0001): ## 外力小于摩擦力，且速度为0，则力和速度归零
                self.particles[i].force = vec2(0.0, 0.0)
                self.particles[i].vel = vec2(0.0, 0.0)
            else:
                fraction = vec2(0.0, 0.0) ## 初始化摩擦力向量
